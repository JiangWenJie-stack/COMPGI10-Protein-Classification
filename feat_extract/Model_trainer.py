...
def load_data(dataFrame="Feat_normalized.csv") :
    ...
    if type(dataFrame) == type(''):
        ...
        #Load file
        df = pd.read_csv(dataFrame, delimiter=',', header=0, encoding="utf-8-sig")
        try:
            #import pdb;pdb.set_trace()
            #df.set_index(keys = ['accession', 'classname'], inplace=True)
            df.set_index(keys = ['classname'], inplace=True)
...
def getClassifier(classifierType):
    if (classifierType == 'SGD'):
        model = SGDClassifier(penalty='elasticnet',class_weight='balanced',n_jobs=-1,n_iter=150,l1_ratio =0.2)
    ...
    if (classifierType == 'SVCrbf'):
       #model = SVC(kernel="rbf", class_weight="balanced", cache_size=1400, shrinking=True)
       ...
       model = GridSearchCV(SVC(C=1, class_weight='balanced',decision_function_shape='ovr', cache_size=1400, shrinking=True), tuned_parameters, cv=2, verbose=1)
       
       
    if (classifierType == 'SVCpoly'):
        model = SVC(kernel="poly", class_weight="balanced", cache_size=1400, shrinking=True)
    return model
