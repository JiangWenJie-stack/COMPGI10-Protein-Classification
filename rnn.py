...
lstmOr = True   # if true then LSTM cell is used instead of GRU
...
class Model:
    ...
    def prediction(self):
        ...
        x = tf.contrib.layers.fully_connected(val, 100, activation_fn=None, biases_initializer=tf.zeros_initializer)
        x = tf.contrib.layers.batch_norm(x, center=True, scale=True, is_training=phase)
        x = tf.nn.relu(x, 'relu')
...
def dataPrep(trains=True,extension="train"):
    ...
    if trains==True:
        ...
        for idx, dat in enumerate(data):
            for seq in dat:
                ...
                np.save(extension+"/onehot",pData)
            print("Done with {}".format(idx))
        return pData
        
    else:
        return np.load(extension+"/onehot.npy")
...
if train == True:
    with tf.Session() as sess:
        ...
        image = tf.placeholder(tf.float32, [None, 200, 21])
        ...
        for j in range(opt_runs):
            ...
            images=images.reshape(-1,200,21)
